...
//    }
    ...
    var firstWeekDayBeforeStart: Date {
        let startOfMonthWeekday = Calendar.current.component(.weekday, from: startOfMonth)
        let adjustment = (startOfMonthWeekday - Self.firstDayOfWeek + 7) % 7
        return Calendar.current.date(byAdding: .day, value: -adjustment, to: startOfMonth)!
    }
    
    var calendarDisplayDays: [Date] {
        var days: [Date] = []
        
        var startDay = firstWeekDayBeforeStart
